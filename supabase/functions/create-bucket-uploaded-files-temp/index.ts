...
Deno.serve(async (req) => {
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for uploaded-files" ON storage.objects FOR SELECT USING (bucket_id = 'uploaded-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for uploaded-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'uploaded-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for uploaded-files" ON storage.objects FOR UPDATE USING (bucket_id = 'uploaded-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for uploaded-files" ON storage.objects FOR DELETE USING (bucket_id = 'uploaded-files');`
    ];
